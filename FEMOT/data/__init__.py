...
from .femot_basedMOT17 import build as build_femot
...
from .dsec_basedMOT17 import build as build_dsec


def build_dataset(config: dict, split: str) -> MOTDataset:
    if config["DATASET"] == "FEMOT":
        return build_femot(config=config, split=split)
    elif config["DATASET"] == "DSEC_distorted":
        return build_dsec(config=config, split=split)
