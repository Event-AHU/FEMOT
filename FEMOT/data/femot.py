...
class FEMOT(MOTDataset):
    def __init__(self, config: dict, split: str, transform):
        ...
        self.images_dir = os.path.join(config["DATA_ROOT"], config["DATASET"], split)
        self.gts_dir = os.path.join(config["DATA_ROOT"], config["DATASET"], split)
        ...
        for vid in os.listdir(self.images_dir):
            ...
            frame_names = os.listdir(self.images_dir)
            frame_names.sort()
            for idx, frame_name in enumerate(frame_names):
                gt_name = frame_name
                # gt_path = os.path.join(self.gts_dir, vid, gt_name)
                gt_path = os.path.join(self.gts_dir, vid, "gt", 'gt.txt')
                # t = int(gt_name[:-4].split("-")[-1])
                t = idx
                if os.path.exists(gt_path):
                    for line in open(gt_path):
                        # c, i, *xywh = line[:-1].split(" ")
                        fid, id, x, y, w, h, score, c, visible = line.strip('\n').split(",")
                        c, i, x, y, w, h = int(c), int(id), float(x), float(y), float(w), float(h)


                        # c, i, *xywh = line.strip('\n').split(",")[:-3]
                        # c, i = map(int, (c, i))
                        # x, y, w, h = map(float, xywh)
                        self.gts[vid][t].append([CLASS_MAPPING[c], i, x, y, w, h])
    ...
    def get_single_frame(self, vid: str, idx: int):
        ...
        img_path = os.path.join(self.images_dir, vid, "fusion", f"{idx:05d}.jpg")
        ...
        info["boxes"][:, 2:] += info["boxes"][:, :2]   # TODO: assert (boxes2[:, 2:] >= boxes2[:, :2]).all()
        # if len(info["boxes"]) > 0:
...
def build(config: dict, split: str):
    if split == "train":
        return FEMOT(config=config, split=split, transform=transfroms_for_train())
    else:
