    ...
    for i, batch in enumerate(dataloader):
        ...
        for frame_idx in range(len(batch["imgs"][0])):
            if no_grad_frames is None or frame_idx >= no_grad_frames:
                ...
                event_frame = [efs[frame_idx] for efs in batch["event_imgs"]]
                for idx, f in enumerate(frame):
                    frame[idx].requires_grad_(False)
                    event_frame[idx].requires_grad_(False)
                frame = tensor_list_to_nested_tensor(tensor_list=frame).to(device)
                event_frame = tensor_list_to_nested_tensor(tensor_list=event_frame).to(device)
                res = model(frame=frame, event_frame=event_frame,tracks=tracks)
        ...
        if multi_checkpoint:
            if i % freq == 0 and is_main_process():
                save_checkpoint(
                    model=model,
                    path=os.path.join(logger.logdir[:-5], f"checkpoint_{int(i // freq)}.pth")
                )
