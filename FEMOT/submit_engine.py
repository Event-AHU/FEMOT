...
class Submitter:
    ...
    def run(self):
        ...
        for i, ((image, ori_image), info, (event_image, event_ori_image), event_info) in enumerate(tqdm(self.dataloader, desc=f"Submit seq: {self.seq_name}")):
            # image: (1, C, H, W); ori_image: (1, H, W, C)
            frame = tensor_list_to_nested_tensor([image[0]]).to(self.device)
            event_frame = tensor_list_to_nested_tensor([event_image[0]]).to(self.device)

            res = self.model(frame=frame, event_frame= event_frame,tracks=tracks)
    ...
    def write_results(self, tracks_result: TrackInstances, frame_idx: int):
        with open(os.path.join(self.predict_dir, f"{self.seq_name}.txt"), "a") as file:
            for i in range(len(tracks_result)):
                # if self.dataset_name == "FEMOT" or self.dataset_name == "VTMOT" or self.dataset_name == "DanceTrack" or self.dataset_name == "SportsMOT" \
